...
(function($) {
	...
	function substitute_text(msg, text) {
		...
		//@todo: need to escape special chars (http://stackoverflow.com/questions/399078/what-special-characters-must-be-escaped-in-regular-expressions)
		//Anywhere: . ^ $ * + - ? ( ) [ ] { } \ |
		var regexp = /{{([^}].)+}}/g;
		var match = regexp.exec(text); // does not seems to work if we don't add the first . search
		console.log('Dashboard::substitute: original: '+text,match);
		while (match != null) {
			...
			if(value != null) {
				text = text.replace(new RegExp(opts.begin_delimiter+varname+opts.end_delimiter, 'g'), value);
			}
	...
	function substitute(msg) { // both title and subject are searched for substitution
		if(msg.payload === null)
			return;
		// subject
		console.log('Dashboard::substitute subject');
		msg.subject_template = msg.subject;
		msg.subject = substitute_text(msg, msg.subject);
		// body
		console.log('Dashboard::substitute body');
