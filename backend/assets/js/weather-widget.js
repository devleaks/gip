...
(function($, ls) {
	...
	WeatherWidget.prototype.makeWidget = function(apiData) {
		...
		var installIcons = new Array();

		var installIcon = function(lid, licon) {
			var ticon = null;
			switch(licon) {
				case 'clear-day': ticon = Skycons.CLEAR_DAY; break;
				case 'clear-night': ticon = Skycons.CLEAR_NIGHT; break;
				case 'cloudy': ticon = Skycons.CLOUDY; break;
				case 'fog': ticon = Skycons.FOG; break;
				case 'partly-cloudy-day': ticon = Skycons.PARTLY_CLOUDY_DAY; break;
				case 'partly-cloudy-night': ticon = Skycons.PARTLY_CLOUDY_NIGHT; break;
				case 'rain': ticon = Skycons.RAIN; break;
				case 'sleet': ticon = Skycons.SLEET; break;
				case 'snow': ticon = Skycons.SNOW; break;
				case 'wind': ticon = Skycons.WIND; break;
			}
			installIcons.push({id: lid, icon: ticon});
		}
		...
		installIcon('skyicons-current', currently.icon);
		...
		for(var i = 0; i < 5; i++) {
			...
			hourClone.find('canvas')
				.attr('id', 'skyicons-hourly-'+i+1);
			installIcon('skyicons-hourly-'+i+1, currently.icon);
		...
		for(var k = 0; k < 7; k++) {
			...
			installIcon('skycons-forecast-'+k, icon);
		}
		...
		//console.log('setting skycons...'+installIcons.length);
		var skycons = new Skycons();
	    for(i = installIcons.length; i--; )
      	  skycons.set(installIcons[i].id, installIcons[i].icon);	    
		skycons.play();
		//console.log('...skycons set');
