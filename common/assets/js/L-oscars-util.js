...
L.Oscars.Util = (function () {
	...
	function getSparkline(feature) {
		...
		var icon = L.divIcon({
			html: isSet(feature.properties._data.options)	? '<span class="'+name+'" data-peity=\''+JSON.stringify(feature.properties._data.options)+'\'>'+data+'</span>'
	...
	function addSidebarTab(options) {
		...
		// 1. Add tab icon
		$('#'+_sidebar_id+" div.sidebar-tabs ul[role='tablist']:nth-child("+options.zone+")")
			.append( $('<li>')
				.append( $('<a>')
					.attr('href', "#"+options.id)
					.attr('role', "tab")
					.attr('title', options.info ? options.info : "")
					.append( $('<i>')
	...
	map: function(args) {
		...
		console.log("L.Oscars.Util::map: created at "+opts.id);
	...
	prepareMap: function(map, args) {
		...
		if(opts.sidebar) {
			...
			var newloc = document.getElementById('layer-control');
		...
		if(opts.sidebar && opts.info) {
			addSidebarTab({
				...
				tab_content: $('<div>')
					...
					.html("Right-click/Control-click on device icon to read more information here.")
			});
			_showInfo = true;
		}
		...
		if(opts.search) {
			addSidebarTab({
				...
				icon: "search",
				icon_extra: null,
				tab_content: '<div id="search-control"></div>'
			});
			...
			var searchControlOptions = opts.sidebar ? {
					...
					marker: {icon: L.icon.pulse(opts.PULSE_ICON), animate: false, circle: false},
				} : {
					layer: _searchLayer,
					propertyName: "name",
					textPlaceholder: 'Search...                ',
...
//				websocket: 'ws://locahost:8051'
		...
		if(opts.sidebar && opts.about) {
			var about = "About Oscars GIP Map";
			var aboutOptions = {
				client: client,
				zone: 2,
				id: "about",
				...
				subtitle: "&nbsp;",
				icon: null,
				icon_extra: '<span class="about ' + client + '" title="' + about + '…"></span>',
				tab_content:
				...
				+'	&copy; 2015—'+(new Date()).getFullYear()+' — Oscars s.a. and respective licensees.'
				+'</div>',
				wrap: false
			};
		...
		// must be done last
		var resetloc = $('.sidebar-tabs a[href="#location"]');
	...
	addBaseLayerToControlLayer: function(layer, layerName, groupName) {
		if(_layerControl) {
			if(_layerControlGrouped) {
				_layerControl.addBaseLayer(layer, layerName, groupName);
			}
			else {
				_layerControl.addBaseLayer(layer, layerName);
			}
		}
	},
